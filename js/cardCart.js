...
const cartBtn = document.querySelector(".cart_box_left_button");
//пробегаемся по базе данных и создаем карточки
data.forEach(({ id, img, name, price, color, size, quantity }) => {
  if (cardArr.includes(id)) {
    const productCartEl = `
        <div class="cart_box_left_card" data-id="${id}">
        <img class="cart_box_left_card_img" src="${img}" alt="${img}">
        <div class="cart_box_left_card_info">
            <a href="product.html" class="cart_box_left_card_info_heading">${name}</a>
    ...
    cartBtn.insertAdjacentHTML("beforebegin", productCartEl);
  }
});
...
//     const indexIdCardDelete = cartArr.indexOf(idCardDelete);
//     cartArr.splice(indexIdCardDelete, 1);
